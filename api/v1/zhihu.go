...
func GetZhihuFavors(ctx *gin.Context) {
	...
	if offsetInt, err := strconv.Atoi(limit); err != nil || offsetInt < 0 {
	...
	//调zhihu api 获取知乎所有收藏夹信息
		resp, err := http.Get(fmt.Sprintf(model.GetFavorsApi + "?limit=%v&offset=%v", limit, offset))
	if err != nil {
		glog.Error(err)
...
func GetZhihuFavorItemsByCid(ctx *gin.Context)  {
	...
	resp, err := http.Get(fmt.Sprintf(model.GetFavorItemsByCidApi + "?limit=%v&offset=%v", cid, limit, offset))
	if err != nil {
		glog.Error(err)
